...
local originalMouseIconVis = uis.MouseIconEnabled
...
local bttn = Instance.new('ImageButton', screenGui)
bttn.Parent = screenGui
bttn.Visible = true
bttn.Modal = true
bttn.Size = UDim2.new(1,0,1,0)
bttn.ZIndex = 9999999999
bttn.Transparency = 1
...
do
	...
	function utility:Create(instanceType, instanceOffset, instanceProperties, instanceParent)
		...
		local instanceOffset = instanceOffset or {
			Vector2.new(0, 0)
		}
		...
		if instance then
			...
			if not instanceHidden then
				...
				library.hidden[#library.hidden + 1] = {
					instance,
					instanceOffset,
					instanceProperties['Transparency'] or 1
				}
...
do
    ...
    --
	function library:Bind(connectionType, connectionCallback)
		local connection = connectionType:Connect(connectionCallback)
		library.connections[#library.connections + 1] = connection
		--
		return connection
	...
	local function Text(text, color)
		local s = DrawingObject('Text', color)
	...
	function library:CreateNotification(t, c)
		local color = c or Color3.new(1,.42,.7)
		local gap = 25
		local width = 18
		local alpha = 255
		local time = 0
		...
		Note.drawings.text = Text(t, color)	
		notes[#notes + 1] = Note
	end
	library:Bind(rs.RenderStepped, function(dt)
	...
	function library:New(info)
        ...
        --
		local main_frame = utility:Create('Frame', {
			Vector2.new(0, 0)
		}, {
        ...
        --
		local frame_inline = utility:Create('Frame', {
			Vector2.new(1, 1),
			main_frame
		}, {
        ...
        --
		local inner_frame = utility:Create('Frame', {
			Vector2.new(1, 1),
			frame_inline
		}, {
        ...
        --
		local inner_frame_inline = utility:Create('Frame', {
			Vector2.new(4, 18),
			inner_frame
		}, {
        ...
        --
		local inner_frame_inline2 = utility:Create('Frame', {
			Vector2.new(1, 1),
			inner_frame_inline
		}, {
        ...
        --
		local back_frame = utility:Create('Frame', {
			Vector2.new(1, 1),
			inner_frame_inline2
		}, {
        ...
        --
		local tab_frame_inline = utility:Create('Frame', {
			Vector2.new(4, 24),
			back_frame
		}, {
        ...
        --
		local tab_frame_inline2 = utility:Create('Frame', {
			Vector2.new(1, 1),
			tab_frame_inline
		}, {
        ...
        --
		local tab_frame = utility:Create('Frame', {
			Vector2.new(1, 1),
			tab_frame_inline2
		}, {
		...
		function window:createConfig()
			local config = {}
			local config_name = library.pointers['config name']:get()
			if config_name == '' then
				return library:CreateNotification('Preset name can\'t be blank.', library.colorpresets.error)
			end
			if isfile(string.format('eclipse/configs/%s.txt', config_name)) then
				return library:CreateNotification(string.format('The file \'%s.txt\' already exists.', config_name), library.colorpresets.error)
			end
			pcall(function()
				for i, v in pairs(library.pointers) do
					if i == 'selected config' or i == 'config name' then
						continue
					end
					if type(v:get()) == 'table' and v:get().Transparency then
						...
						config[i] = {
							...
							Transparency = v:get().Transparency
						}
			...
			do
				writefile(string.format('eclipse/configs/%s.txt', config_name), hs:JSONEncode(config))
				writefile(string.format('eclipse/configs/backups/%s.txt', config_name), hs:JSONEncode(config))
				library:CreateNotification(string.format('Preset \'%s\' successfully saved!', config_name), library.colorpresets.success)
			end
		end
        --
        --
		function window:saveConfig()
			local config = {}
			local config_name = library.pointers['selected config']:get()[1][1]
			if config_name == '' then
				return library:CreateNotification('Please select a preset to save.' ,library.colorpresets.error)
			end
			pcall(function()
				for i, v in pairs(library.pointers) do
					if i == 'selected config' or i == 'config name' then
			...
			delfile(string.format('eclipse/configs/%s.txt', config_name))
			wait()
			writefile(string.format('eclipse/configs/%s.txt', config_name), hs:JSONEncode(config))
			library:CreateNotification(string.format('Preset \'%s\' successfully saved!', config_name), library.colorpresets.success)
		end
		function window:loadConfig()
			local config_name = library.pointers['selected config']:get()[1][1]
			if not isfile(string.format('eclipse/configs/%s.txt', config_name)) then
				return library:CreateNotification(string.format('Preset \'%s\' can\'t be found.', config_name), library.colorpresets.error)
			end
			local config = hs:JSONDecode(readfile(string.format('eclipse/configs/%s.txt', config_name)))
            --
			...
			library:CreateNotification(string.format('Preset \'%s\' loaded!', config_name), library.colorpresets.success)
		end
		function window:updateConfigs()
			local list = {}
			for idx, file in ipairs(listfiles('eclipse/configs')) do
				local file_name = file:gsub('eclipse/configs\\', ''):gsub('.txt', '')
				if file_name == 'backups' then
					continue
				end
				list[#list + 1] = file_name
			end;
			local is_new = #list ~= #current_configs;
			if not is_new then
				for idx, file in ipairs(list) do
					if file ~= current_configs[idx] then
						is_new = true;
						break
					end
				end
			...
			if is_new then
				...
				library.pointers['selected config']:UpdateList(list)
			end
		end;
		...
		function window:Unload()
			uis.MouseIconEnabled = originalMouseIconVis
            screenGui:Destroy()
			for i, v in pairs(library.connections) do
				v:Disconnect()
				v = nil
		...
		function window:Watermark(info)
            ...
            --
			local info = info or {}
			local watermark_name = info.name or info.Name or info.title or info.Title or 'Watermark'--string.format('$$ Splix || uid : %u || ping : %u || fps : %u', 1, 100, 200)
            --
			local text_bounds = utility:GetTextBounds(watermark_name, theme.textsize, theme.font)
            --
			local watermark_frame = utility:Create('Frame', {
				Vector2.new(30, 9)
			}, {
				Size = utility:Size(0, text_bounds.X + 20, 0, 21),
				Position = utility:Position(0, 30, 0, 9),
				Transparency = 0.8,
				Hidden = true,
				ZIndex = 1010,
				Color = theme.darkcontrast,
			...
			window.watermark.outline = watermark_frame
            --
            ...
            --
            --
			local watermark_accent = utility:Create('Frame', {
				Vector2.new(0, 0),
				watermark_frame
			}, {
				Size = utility:Size(1, 0, 0, 1, watermark_frame),
				Position = utility:Position(0, 0, 0, 1, watermark_frame),
				Hidden = true,
				ZIndex = 1010,
				Color = theme.accent,
				Visible = window.watermark.visible
            ...
            --
			local watermark_title = utility:Create('TextLabel', {
				Vector2.new(8, 4),
				watermark_frame
			}, {
				Text = '',
				...
				Hidden = true,
				ZIndex = 1010,
				Position = utility:Position(0, 8, 0, 4, watermark_frame),
				Visible = window.watermark.visible
			})
            --
			...
			function window.watermark:UpdateSize()
				watermark_frame.Size = utility:Size(0, watermark_title.TextBounds.X + 4 + (12), 0, 21)
			...
			function window.watermark:UpdatePosition()
				watermark_frame.Position = utility:Position(0, 30, 0, 9)
                watermark_accent.Position = utility:Position(0, 0, 0, 1, watermark_frame)
				watermark_title.Position = utility:Position(0, 8, 0, 4, watermark_frame)
			end
			--
			function getkdr(plr)
				if not plr then plr = plrs.LocalPlayer end
				if not plr:FindFirstChild('Status') then 
					return nil end
				local kills, deaths = plr.Status.Kills.Value, plr.Status.Deaths.Value
				if deaths <= 0 then deaths = 1 end
				return string.format('%.2f',(tostring(kills)/tostring(deaths)))
			end
            --
			...
			utility:Connection(rs.RenderStepped, function()
				if watermark_frame.Visible then
					library.shared.ping = stats.Network:FindFirstChild('ServerStatsItem') and tostring(math.floor(stats.Network.ServerStatsItem['Data Ping']:GetValue())) or '??'
					library.shared.fps  = stats.FrameRateManager:FindFirstChild('RenderAverage') and string.format('%.1f', 1000 / stats.FrameRateManager.RenderAverage:GetValue()) or '??'
					library.shared.uid = getgenv().uid or '??'
					local date = os.date'%X'
					window.watermark:UpdatePosition()
					--
					if (tick() - Tick) > 0.03 then
						watermark_title.Text = string.format(' eclipse | uid: %s | %s fps | %s ms | %s kd/r | %s', library.shared.uid, library.shared.fps, library.shared.ping, getkdr(), date)
						window.watermark:UpdateSize()
						--
						Tick = tick()
					end
		...
		function window:SpectatorList(info)
			...
			--
			local info = info or {}
			local speclist_name = info.name or info.Name or info.title or info.Title or 'Spectators'
			--
			local text_bounds = utility:GetTextBounds(speclist_name, theme.textsize, theme.font)
			--
			local speclist_frame = utility:Create('Frame', {
				Vector2.new(0, 0)
			}, {
				Size = utility:Size(0, text_bounds.X + 20, 0, 21),
				Position = utility:Position(0, 3, 0, 340),
				Transparency = 0.8,
				Hidden = true,
				ZIndex = 1010,
				Color = theme.darkcontrast,
				Visible = window.speclist.visible
			})
			window.speclist.outline = speclist_frame
			--
			library.colors[speclist_frame] = {
				Color = 'darkcontrast'
			}
			--
			--
			local speclist_accent = utility:Create('Frame', {
				Vector2.new(0, 0),
				speclist_frame
			}, {
				Size = utility:Size(1, 0, 0, 1, speclist_frame),
				Position = utility:Position(0, 0, 0, 1, speclist_frame),
				Hidden = true,
				ZIndex = 1010,
				Color = theme.accent,
				Visible = window.speclist.visible
			...
			--
			library.colors[speclist_accent] = {
				Color = 'accent'
			}
			--
			local speclist_title = utility:Create('TextLabel', {
				Vector2.new(8, 4),
				speclist_frame
			}, {
				Text = '',
				...
				Center = true,
				ZIndex = 1010,
				Position = utility:Position(0.5, 0, 0, 4, speclist_frame),
				Visible = window.speclist.visible
			})
			--
			library.colors[speclist_title] = {
				OutlineColor = 'textborder',
				Color = 'textcolor'
			}
			--
			function window.speclist:UpdateSize()
				speclist_frame.Size = utility:Size(0, speclist_title.TextBounds.X + 4 + (12), 0, speclist_title.TextBounds.Y + 4 + (5))
				speclist_accent.Size = utility:Size(1, 0, 0, 1, speclist_frame)
			end
			--
			function window.speclist:Visibility()
				speclist_frame.Visible = window.speclist.visible
				speclist_accent.Visible = window.speclist.visible
				speclist_title.Visible = window.speclist.visible
			end
			--
			function window.speclist:Update(updateType, updateValue)
				if updateType == 'Visible' then
					window.speclist.visible = updateValue
					window.speclist:Visibility()
				end
				if updateType == 'Info' then
					speclist_title.Text = updateValue
				end
				if updateType == 'Position' then
					speclist_frame.Position = utility:Position(updateValue[1], updateValue[2], updateValue[3], updateValue[4])
					speclist_accent.Position = utility:Position(0, 0, 0, 1, speclist_frame)
					speclist_title.Position = utility:Position(0.5, 0, 0, 4, speclist_frame)
				end
			end
			--
			utility:Connection(rs.RenderStepped, function()
				if speclist_frame.Visible then
					window.speclist:UpdateSize()
					window.speclist:Update('Position', {0, 3, 0, 340})
				end
			end)
			--
			window.speclist:UpdateSize()
			--
			return window.speclist
		end
        --
		function window:Indicator(info)
			window.indicator = {
				visible = false
			}
            --
			local info = info or {}
			local indicator_name = info.name or info.Name or info.title or info.Title or 'Indicator'
            --
			local text_bounds = utility:GetTextBounds(indicator_name, theme.textsize, theme.font)
            --
			local indicator_frame = utility:Create('Frame', {
				Vector2.new(0, 0)
			}, {
				Size = utility:Size(0, text_bounds.X + 20, 0, 21),
				Position = utility:Position(0, 3, 0, 305),
				Transparency = 0.8,
				Hidden = true,
				ZIndex = 1010,
				Color = theme.darkcontrast,
				Visible = window.indicator.visible
			})
			window.indicator.outline = indicator_frame
            --
			library.colors[indicator_frame] = {
				Color = 'darkcontrast'
			}
            --
            --
			local indicator_accent = utility:Create('Frame', {
				Vector2.new(0, 0),
				indicator_frame
			}, {
				Size = utility:Size(1, 0, 0, 1, indicator_frame),
				Position = utility:Position(0, 0, 0, 1, indicator_frame),
				Hidden = true,
				ZIndex = 1010,
				Color = theme.accent,
				Visible = window.indicator.visible
            ...
            --
			library.colors[indicator_accent] = {
				Color = 'accent'
			}
            --
			local indicator_title = utility:Create('TextLabel', {
				Vector2.new(8, 4),
				indicator_frame
			}, {
				Text = '',
				...
				Hidden = true,
				ZIndex = 1010,
				Position = utility:Position(0, 8, 0, 4, indicator_frame),
				Visible = window.indicator.visible
			})
            --
			library.colors[indicator_title] = {
				OutlineColor = 'textborder',
				Color = 'textcolor'
			}
            --
			function window.indicator:UpdateSize()
				indicator_frame.Size = utility:Size(0, indicator_title.TextBounds.X + 4 + (12), 0, 21)
				indicator_accent.Size = utility:Size(1, 0, 0, 1, indicator_frame)
			end
            --
			function window.indicator:Visibility()
				indicator_frame.Visible = window.indicator.visible
				indicator_accent.Visible = window.indicator.visible
				indicator_title.Visible = window.indicator.visible
			end
            --
			function window.indicator:Update(updateType, updateValue)
				if updateType == 'Visible' then
					window.indicator.visible = updateValue
					window.indicator:Visibility()
				end
				if updateType == 'Info' then
					indicator_title.Text = updateValue
				end
				if updateType == 'Position' then
					indicator_frame.Position = utility:Position(updateValue[1], updateValue[2], updateValue[3], updateValue[4])
					indicator_accent.Position = utility:Position(0, 0, 0, 1, indicator_frame)
					indicator_title.Position = utility:Position(0, 8, 0, 4, indicator_frame)
				end
			end
			--
			utility:Connection(rs.RenderStepped, function()
				window.indicator:UpdateSize()
			end)
            --
			window.indicator:UpdateSize()
            --
		...
		function window:KeybindsList(info)
			...
			local info = info or {}
            --
			local keybindslist_outline = utility:Create('Frame', {
				Vector2.new(10, (utility:GetScreenSize().Y / 2) - 200)
			}, {
            ...
            --
			local keybindslist_inline = utility:Create('Frame', {
				Vector2.new(1, 1),
				keybindslist_outline
			}, {
            ...
            --
			local keybindslist_frame = utility:Create('Frame', {
				Vector2.new(1, 1),
				keybindslist_inline
			}, {
            ...
            --
			local keybindslist_accent = utility:Create('Frame', {
				Vector2.new(0, 0),
				keybindslist_frame
			}, {
            ...
            --
			local keybindslist_title = utility:Create('TextLabel', {
				Vector2.new(keybindslist_outline.Size.X / 2, 4),
				keybindslist_outline
			}, {
				Text = 'Keybinds',
				Size = theme.textsize,
			...
			function window.keybindslist:Add(keybindname, keybindvalue)
				if keybindname and keybindvalue and not window.keybindslist.keybinds[keybindname] then
					...
					local keybind_outline = utility:Create('Frame', {
						Vector2.new(0, keybindslist_outline.Size.Y - 1),
						keybindslist_outline
					}, {
                    ...
                    --
					local keybind_inline = utility:Create('Frame', {
						Vector2.new(1, 1),
						keybind_outline
					}, {
                    ...
                    --
					local keybind_frame = utility:Create('Frame', {
						Vector2.new(1, 1),
						keybind_inline
					}, {
                    ...
                    --
					local keybind_title = utility:Create('TextLabel', {
						Vector2.new(4, 3),
						keybind_outline
					}, {
						Text = keybindname,
                    ...
                    --
					local keybind_value = utility:Create('TextLabel', {
						Vector2.new(keybind_outline.Size.X - 4 - utility:GetTextBounds(keybindname, theme.textsize, theme.font).X, 3),
						keybind_outline
					}, {
						Text = '[' .. keybindvalue .. ']',
						Size = theme.textsize,
		...
		function window:StatusList(info)
			...
			local info = info or {}
            --
			local statuslist_outline = utility:Create('Frame', {
				Vector2.new(10, (utility:GetScreenSize().Y / 2) - 200)
			}, {
            ...
            --
			local statuslist_inline = utility:Create('Frame', {
				Vector2.new(1, 1),
				statuslist_outline
			}, {
            ...
            --
			local statuslist_frame = utility:Create('Frame', {
				Vector2.new(1, 1),
				statuslist_inline
			}, {
            ...
            --
			local statuslist_accent = utility:Create('Frame', {
				Vector2.new(0, 0),
				statuslist_frame
			}, {
            ...
            --
			local statuslist_title = utility:Create('TextLabel', {
				Vector2.new(statuslist_outline.Size.X / 2, 4),
				statuslist_outline
			}, {
				Text = 'Status',
				Size = theme.textsize,
			...
			function window.statuslist:Add(statusname)
				if statusname and not window.statuslist.statuses[statusname] then
					local statusTable = {}
                    --
					local status_outline = utility:Create('Frame', {
						Vector2.new(0, statuslist_outline.Size.Y - 1),
						statuslist_outline
					}, {
                    ...
                    --
					local status_inline = utility:Create('Frame', {
						Vector2.new(1, 1),
						status_outline
					}, {
                    ...
                    --
					local status_frame = utility:Create('Frame', {
						Vector2.new(1, 1),
						status_inline
					}, {
                    ...
                    --
					local status_title = utility:Create('TextLabel', {
						Vector2.new(4, 3),
						status_outline
					}, {
						Text = statusname,
		...
		function window:Fade()
			...
			--print(originalMouseBehavior)
			window.fading = true
			window.isVisible = not window.isVisible
            screenGui.Enabled = window.isVisible;
            --
			spawn(function()
				for i, v in pairs(library.drawings) do
					utility:Lerp(v[1], {
						Transparency = window.isVisible and v[3] or 0
					}, 0.25)
				end
			end)
            --
		...
		function window:Initialize()
			...
			window:Cursor()
			window:Indicator()
			window:SpectatorList()
            --
			window:Fade()
		end
        --
		library.began[#library.began + 1] = function(Input)
			if Input.UserInputType == Enum.UserInputType.MouseButton1 and window.isVisible and window.isVisible and utility:MouseOverDrawing({
				main_frame.Position.X,
				main_frame.Position.Y,
				main_frame.Position.X + main_frame.Size.X,
				main_frame.Position.Y + 20
			}) then
				local mouseLocation = utility:MouseLocation()
                --
	...
	function library:Page(info)
        ...
        --
		local page_button = utility:Create('Frame', {
			Vector2.new(position, 4),
			window.back_frame
		}, {
        ...
        --
		local page_button_inline = utility:Create('Frame', {
			Vector2.new(1, 1),
			page_button
		}, {
        ...
        --
		local page_button_color = utility:Create('Frame', {
			Vector2.new(1, 1),
			page_button_inline
		}, {
        ...
        --
		local page_button_title = utility:Create('TextLabel', {
			Vector2.new(utility:Position(0.5, 0, 0, 2, page_button_color).X - page_button_color.Position.X, 2),
			page_button_color
		}, {
			Text = name,
			Size = theme.textsize,
		...
		function page:Update()
			...
			for i, v in pairs(page.sections) do
				utility:UpdateOffset(v.section_inline, {
					Vector2.new(v.side == 'right' and (window.tab_frame.Size.X / 2) + 2 or 5, 5 + page['sectionOffset'][v.side]),
					window.tab_frame
				})
        ...
        --
		library.began[#library.began + 1] = function(Input)
			if Input.UserInputType == Enum.UserInputType.MouseButton1 and window.isVisible and utility:MouseOverDrawing({
				page_button.Position.X,
				page_button.Position.Y,
				page_button.Position.X + page_button.Size.X,
				page_button.Position.Y + page_button.Size.Y
	...
	function pages:Section(info)
		...
		local section_inline = utility:Create('Frame', {
			Vector2.new(side == 'right' and (window.tab_frame.Size.X / 2) + 2 or 5, 5 + page['sectionOffset'][side]),
			window.tab_frame
		}, {
		...
		section['section_inline'] = section_inline
        --
		library.colors[section_inline] = {
			Color = 'inline'
		}
        --
		local section_outline = utility:Create('Frame', {
			Vector2.new(1, 1),
			section_inline
		}, {
		...
		section['section_outline'] = section_outline
        --
		library.colors[section_outline] = {
			Color = 'outline'
		}
        --
		local section_frame = utility:Create('Frame', {
			Vector2.new(1, 1),
			section_outline
		}, {
		...
		section['section_frame'] = section_frame
        --
		library.colors[section_frame] = {
			Color = 'darkcontrast'
		}
        --
		local section_accent = utility:Create('Frame', {
			Vector2.new(0, 0),
			section_frame
		}, {
		...
		section['section_accent'] = section_accent
        --
	...
	function pages:MultiSection(info)
        ...
        --
		local multiSection_inline = utility:Create('Frame', {
			Vector2.new(side == 'right' and (window.tab_frame.Size.X / 2) + 2 or 5, 5 + page['sectionOffset'][side]),
			window.tab_frame
		}, {
		...
		multiSection['section_inline'] = multiSection_inline
        --
		library.colors[multiSection_inline] = {
			Color = 'inline'
		}
        --
		local multiSection_outline = utility:Create('Frame', {
			Vector2.new(1, 1),
			multiSection_inline
		}, {
		...
		multiSection['section_outline'] = multiSection_outline
        --
		library.colors[multiSection_outline] = {
			Color = 'outline'
		}
        --
		local multiSection_frame = utility:Create('Frame', {
			Vector2.new(1, 1),
			multiSection_outline
		}, {
		...
		multiSection['section_frame'] = multiSection_frame
        --
		library.colors[multiSection_frame] = {
			Color = 'darkcontrast'
		}
        --
		local multiSection_backFrame = utility:Create('Frame', {
			Vector2.new(0, 2),
			multiSection_frame
		}, {
        ...
        --
		local multiSection_bottomFrame = utility:Create('Frame', {
			Vector2.new(0, multiSection_backFrame.Size.Y - 1),
			multiSection_backFrame
		}, {
        ...
        --
		local multiSection_accent = utility:Create('Frame', {
			Vector2.new(0, 0),
			multiSection_frame
		}, {
		...
		multiSection['section_accent'] = multiSection_accent
        --
		...
		for i, v in pairs(msections) do
			local msection = {
				...
				sections = {},
				visibleContent = {},
				section_inline = multiSection_inline,
				section_outline = multiSection_outline,
				section_frame = multiSection_frame,
				section_accent = multiSection_accent
			}
            --
			local textBounds = utility:GetTextBounds(v, theme.textsize, theme.font)
            --
			local msection_frame = utility:Create('Frame', {
				Vector2.new(multiSection.xAxis, 0),
				multiSection_backFrame
			}, {
			...
			msection['msection_frame'] = msection_frame
            --
			library.colors[msection_frame] = {
				Color = i == 1 and 'darkcontrast' or 'lightcontrast'
			}
            --
			local msection_line = utility:Create('Frame', {
				Vector2.new(msection_frame.Size.X, 0),
				msection_frame
			}, {
            ...
            --
			local msection_title = utility:Create('TextLabel', {
				Vector2.new(msection_frame.Size.X * 0.5, 1),
				msection_frame
			}, {
				Text = v,
				Size = theme.textsize,
            ...
            --
			local msection_bottomline = utility:Create('Frame', {
				Vector2.new(0, msection_frame.Size.Y),
				msection_frame
			}, {
			...
			msection['msection_bottomline'] = msection_bottomline
            --
            ...
            --
			library.began[#library.began + 1] = function(Input)
				if Input.UserInputType == Enum.UserInputType.MouseButton1 and window.isVisible and page.open and  utility:MouseOverDrawing({
					msection_frame.Position.X,
					msection_frame.Position.Y,
					msection_frame.Position.X + msection_frame.Size.X,
					msection_frame.Position.Y + msection_frame.Size.Y
				}) and multiSection.currentSection ~= msection and not window:IsOverContent() then
	...
	function sections:Label(info)
        ...
        --
		local label_title = utility:Create('TextLabel', {
			Vector2.new(middle and (section.section_frame.Size.X / 2) or 4, label.axis),
	...
	function sections:Textbox(info)
		...
		local callback = info.callback or info.Callback or function()
		end
        --
		local window = self.window
		local page = self.page
        ...
        --
		local textbox_outline = utility:Create('Frame', {
			Vector2.new(4, textbox.axis),
			section.section_frame
		}, {
        ...
        --
		local textbox_inline = utility:Create('Frame', {
			Vector2.new(1, 1),
			textbox_outline
		}, {
        ...
        --
		local textbox_frame = utility:Create('Frame', {
			Vector2.new(1, 1),
			textbox_inline
		}, {
        ...
        --
		local button_gradient = utility:Create('Image', {
			Vector2.new(0, 0),
			textbox_frame
		}, {
        ...
        --
		local placeholder_text = utility:Create('TextLabel', {
			Vector2.new(middle and textbox_frame.Size.X / 2 or 4, 0),
			textbox_frame
		}, {
			Text = placeholder,
			Size = theme.textsize,
	...
	function sections:Toggle(info)
        ...
        --
		local toggle_outline = utility:Create('Frame', {
			Vector2.new(4, toggle.axis),
			section.section_frame
		}, {
        ...
        --
		local toggle_inline = utility:Create('Frame', {
			Vector2.new(1, 1),
			toggle_outline
		}, {
        ...
        --
		local toggle_frame = utility:Create('Frame', {
			Vector2.new(1, 1),
			toggle_inline
		}, {
        ...
        --
		local toggle__gradient = utility:Create('Image', {
			Vector2.new(0, 0),
			toggle_frame
		}, {
		...
		local toggle_title = utility:Create('TextLabel', {
			Vector2.new(23, toggle.axis + (7.5) - (utility:GetTextBounds(name, theme.textsize, theme.font).Y / 2)),
			section.section_frame
		}, {
			Text = name,
			Size = theme.textsize,
			...
			Color = theme.textcolor,
			OutlineColor = theme.textborder,
			Position = utility:Position(0, 23, 0, toggle.axis + (7.5) - (utility:GetTextBounds(name, theme.textsize, theme.font).Y / 2), section.section_frame),
			Visible = page.open
		}, section.visibleContent)
        --
        ...
        --
		library.began[#library.began + 1] = function(Input)
			if Input.UserInputType == Enum.UserInputType.MouseButton1 and toggle_outline.Visible and window.isVisible and page.open and utility:MouseOverDrawing({
				...
				section.section_frame.Position.Y + toggle.axis + 15
		...
		function toggle:Colorpicker(info)
			local info = info or {}
			local cpinfo = info.info or info.Info or info.name or name
			...
			local pointer = info.pointer or info.Pointer or info.flag or info.Flag or nil
			local callback = info.callback or info.callBack or info.Callback or info.CallBack or function()
			end
            --
            ...
            --
			local colorpicker_outline = utility:Create('Frame', {
				Vector2.new(section.section_frame.Size.X - (toggle.colorpickers == 0 and (34) or (68)), colorpicker.axis),
				section.section_frame
			}, {
				Size = utility:Size(0, 30, 0, 15),
				Position = utility:Position(1, -(toggle.colorpickers == 0 and (34) or (68)), 0, colorpicker.axis, section.section_frame),
				Color = theme.outline,
				Visible = page.open
			}, section.visibleContent)
            --
            ...
            --
			local colorpicker_inline = utility:Create('Frame', {
				Vector2.new(1, 1),
				colorpicker_outline
			}, {
			...
			if transp then
				colorpicker__transparency = utility:Create('Image', {
					Vector2.new(1, 1),
					colorpicker_inline
				}, {
            ...
            --
			local colorpicker_frame = utility:Create('Frame', {
				Vector2.new(1, 1),
				colorpicker_inline
			}, {
				Size = utility:Size(1, -2, 1, -2, colorpicker_inline),
            ...
            --
			local colorpicker__gradient = utility:Create('Image', {
				Vector2.new(0, 0),
				colorpicker_frame
			}, {
			...
			function colorpicker:Refresh()
				...
				if colorpicker.open and colorpicker.holder.picker and colorpicker.holding.picker then
                    ...
                    --
					utility:UpdateOffset(colorpicker.holder.picker_cursor, {
						Vector2.new((colorpicker.holder.picker.Size.X * colorpicker.current[2]) - 3, (colorpicker.holder.picker.Size.Y * (1 - colorpicker.current[3])) - 3),
						colorpicker.holder.picker
					})
                    --
				...
				elseif colorpicker.open and colorpicker.holder.huepicker and colorpicker.holding.huepicker then
                    ...
                    --
					utility:UpdateOffset(colorpicker.holder.huepicker_cursor[1], {
						Vector2.new(-3, (colorpicker.holder.huepicker.Size.Y * colorpicker.current[1]) - 3),
						colorpicker.holder.huepicker
					})
                    --
				...
				elseif colorpicker.open and colorpicker.holder.transparency and colorpicker.holding.transparency then
					...
					utility:UpdateOffset(colorpicker.holder.transparency_cursor[1], {
						Vector2.new((colorpicker.holder.transparency.Size.X * (1 - colorpicker.current[4])) - 3, -3),
						colorpicker.holder.transparency
					})
                    --
					colorpicker.holder.background.Color = Color3.fromHSV(colorpicker.current[1], 1, 1)
				end
			...
			function colorpicker:get()
				return {
					...
					Transparency = colorpicker.current[4]
				}
			end
            --
			library.began[#library.began + 1] = function(Input)
				if Input.UserInputType == Enum.UserInputType.MouseButton1 and window.isVisible and colorpicker_outline.Visible then
					if colorpicker.open and colorpicker.holder.inline and utility:MouseOverDrawing({
						colorpicker.holder.inline.Position.X,
						colorpicker.holder.inline.Position.Y,
						colorpicker.holder.inline.Position.X + colorpicker.holder.inline.Size.X,
						colorpicker.holder.inline.Position.Y + colorpicker.holder.inline.Size.Y
					}) then
						if colorpicker.holder.picker and utility:MouseOverDrawing({
							colorpicker.holder.picker.Position.X - 2,
							colorpicker.holder.picker.Position.Y - 2,
							colorpicker.holder.picker.Position.X - 2 + colorpicker.holder.picker.Size.X + 4,
							colorpicker.holder.picker.Position.Y - 2 + colorpicker.holder.picker.Size.Y + 4
						}) then
							colorpicker.holding.picker = true
							colorpicker:Refresh()
						elseif colorpicker.holder.huepicker and utility:MouseOverDrawing({
							colorpicker.holder.huepicker.Position.X - 2,
							colorpicker.holder.huepicker.Position.Y - 2,
							colorpicker.holder.huepicker.Position.X - 2 + colorpicker.holder.huepicker.Size.X + 4,
							colorpicker.holder.huepicker.Position.Y - 2 + colorpicker.holder.huepicker.Size.Y + 4
						}) then
							colorpicker.holding.huepicker = true
							colorpicker:Refresh()
						elseif colorpicker.holder.transparency and utility:MouseOverDrawing({
							colorpicker.holder.transparency.Position.X - 2,
							colorpicker.holder.transparency.Position.Y - 2,
							colorpicker.holder.transparency.Position.X - 2 + colorpicker.holder.transparency.Size.X + 4,
							colorpicker.holder.transparency.Position.Y - 2 + colorpicker.holder.transparency.Size.Y + 4
						}) then
							colorpicker.holding.transparency = true
							colorpicker:Refresh()
						end
					elseif utility:MouseOverDrawing({
						section.section_frame.Position.X + (section.section_frame.Size.X - (colorpicker.index == 0 and (36) or (70))),
						section.section_frame.Position.Y + colorpicker.axis,
						section.section_frame.Position.X + section.section_frame.Size.X - (colorpicker.index == 1 and 36 or 0),
						section.section_frame.Position.Y + colorpicker.axis + 15
					}) and not window:IsOverContent() then
						if not colorpicker.open then
							window:CloseContent()
							colorpicker.open = not colorpicker.open
                            --
							local colorpicker_open_outline = utility:Create('Frame', {
								Vector2.new(4, colorpicker.axis + 19),
								section.section_frame
							}, {
							...
							colorpicker.holder.inline = colorpicker_open_outline
                            --
							library.colors[colorpicker_open_outline] = {
								Color = 'outline'
							}
                            --
							local colorpicker_open_inline = utility:Create('Frame', {
								Vector2.new(1, 1),
								colorpicker_open_outline
							}, {
                            ...
                            --
							local colorpicker_open_frame = utility:Create('Frame', {
								Vector2.new(1, 1),
								colorpicker_open_inline
							}, {
                            ...
                            --
							local colorpicker_open_accent = utility:Create('Frame', {
								Vector2.new(0, 0),
								colorpicker_open_frame
							}, {
                            ...
                            --
							local colorpicker_title = utility:Create('TextLabel', {
								Vector2.new(4, 2),
								colorpicker_open_frame
							}, {
								Text = cpinfo,
                            ...
                            --
							local colorpicker_open_picker_outline = utility:Create('Frame', {
								Vector2.new(4, 17),
								colorpicker_open_frame
							}, {
                            ...
                            --
							local colorpicker_open_picker_inline = utility:Create('Frame', {
								Vector2.new(1, 1),
								colorpicker_open_picker_outline
							}, {
                            ...
                            --
							local colorpicker_open_picker_bg = utility:Create('Frame', {
								Vector2.new(1, 1),
								colorpicker_open_picker_inline
							}, {
							...
							colorpicker.holder.background = colorpicker_open_picker_bg
                            --
							local colorpicker_open_picker_image = utility:Create('Image', {
								Vector2.new(0, 0),
								colorpicker_open_picker_bg
							}, {
								Size = utility:Size(1, 0, 1, 0, colorpicker_open_picker_bg),
								Position = utility:Position(0, 0, 0 , 0, colorpicker_open_picker_bg),
							}, colorpicker.holder.drawings);
							colorpicker.holder.picker = colorpicker_open_picker_image
                            --
							local colorpicker_open_picker_cursor = utility:Create('Image', {
								Vector2.new((colorpicker_open_picker_image.Size.X * colorpicker.current[2]) - 3, (colorpicker_open_picker_image.Size.Y * (1 - colorpicker.current[3])) - 3),
							...
							colorpicker.holder.picker_cursor = colorpicker_open_picker_cursor
                            --
							local colorpicker_open_huepicker_outline = utility:Create('Frame', {
								Vector2.new(colorpicker_open_frame.Size.X - 19, 17),
								colorpicker_open_frame
							}, {
                            ...
                            --
							local colorpicker_open_huepicker_inline = utility:Create('Frame', {
								Vector2.new(1, 1),
								colorpicker_open_huepicker_outline
							}, {
                            ...
                            --
							local colorpicker_open_huepicker_image = utility:Create('Image', {
								Vector2.new(1, 1),
								colorpicker_open_huepicker_inline
							}, {
							...
							colorpicker.holder.huepicker = colorpicker_open_huepicker_image
                            --
							local colorpicker_open_huepicker_cursor_outline = utility:Create('Frame', {
								Vector2.new(-3, (colorpicker_open_huepicker_image.Size.Y * colorpicker.current[1]) - 3),
							...
							colorpicker.holder.huepicker_cursor[1] = colorpicker_open_huepicker_cursor_outline
                            --
							library.colors[colorpicker_open_huepicker_cursor_outline] = {
								Color = 'outline'
							}
                            --
							local colorpicker_open_huepicker_cursor_inline = utility:Create('Frame', {
								Vector2.new(1, 1),
								colorpicker_open_huepicker_cursor_outline
							}, {
							...
							colorpicker.holder.huepicker_cursor[2] = colorpicker_open_huepicker_cursor_inline
                            --
							library.colors[colorpicker_open_huepicker_cursor_inline] = {
								Color = 'textcolor'
							}
                            --
							local colorpicker_open_huepicker_cursor_color = utility:Create('Frame', {
								Vector2.new(1, 1),
								colorpicker_open_huepicker_cursor_inline
							}, {
							...
							colorpicker.holder.huepicker_cursor[3] = colorpicker_open_huepicker_cursor_color
                            --
							if transp then
								local colorpicker_open_transparency_outline = utility:Create('Frame', {
									Vector2.new(4, colorpicker_open_frame.Size.X - 19),
									colorpicker_open_frame
								}, {
                                ...
                                --
								local colorpicker_open_transparency_inline = utility:Create('Frame', {
									Vector2.new(1, 1),
									colorpicker_open_transparency_outline
								}, {
                                ...
                                --
								local colorpicker_open_transparency_bg = utility:Create('Frame', {
									Vector2.new(1, 1),
									colorpicker_open_transparency_inline
								}, {
								...
								colorpicker.holder.transparencybg = colorpicker_open_transparency_bg
                                --
								local colorpicker_open_transparency_image = utility:Create('Image', {
									Vector2.new(1, 1),
									colorpicker_open_transparency_inline
								}, {
									Size = utility:Size(1, -2, 1, -2, colorpicker_open_transparency_inline),
								...
								colorpicker.holder.transparency = colorpicker_open_transparency_image
                                --
								local colorpicker_open_transparency_cursor_outline = utility:Create('Frame', {
									Vector2.new((colorpicker_open_transparency_image.Size.X * (1 - colorpicker.current[4])) - 3, -3),
									colorpicker_open_transparency_image
								}, {
								...
								colorpicker.holder.transparency_cursor[1] = colorpicker_open_transparency_cursor_outline
                                --
								library.colors[colorpicker_open_transparency_cursor_outline] = {
									Color = 'outline'
								}
                                --
								local colorpicker_open_transparency_cursor_inline = utility:Create('Frame', {
									Vector2.new(1, 1),
									colorpicker_open_transparency_cursor_outline
								}, {
								...
								colorpicker.holder.transparency_cursor[2] = colorpicker_open_transparency_cursor_inline
                                --
								library.colors[colorpicker_open_transparency_cursor_inline] = {
									Color = 'textcolor'
								}
                                --
								local colorpicker_open_transparency_cursor_color = utility:Create('Frame', {
									Vector2.new(1, 1),
									colorpicker_open_transparency_cursor_inline
								}, {
								...
								colorpicker.holder.transparency_cursor[3] = colorpicker_open_transparency_cursor_color
                                --
		...
		function toggle:Keybind(info)
            ...
            --
			local keybind_outline = utility:Create('Frame', {
				Vector2.new(section.section_frame.Size.X - (44), keybind.axis),
				section.section_frame
			}, {
				Size = utility:Size(0, 40, 0, 17),
				Position = utility:Position(1, -44, 0, keybind.axis, section.section_frame),
				Color = theme.outline,
				Visible = page.open
			}, section.visibleContent)
            --
            ...
            --
			local keybind__gradient = utility:Create('Image', {
				Vector2.new(0, 0),
				keybind_frame
			}, {
            ...
            --
			local keybind_value = utility:Create('TextLabel', {
				Vector2.new(keybind_outline.Size.X / 2, 1),
				keybind_outline
			}, {
				Text = '...',
				Size = theme.textsize,
				...
				if input.EnumType then
					if input.EnumType == Enum.KeyCode or input.EnumType == Enum.UserInputType then
						if table.find(allowedKeyCodes, input.Name) or table.find(allowedInputTypes, input.Name) then
							inputTable = {
								input.EnumType == Enum.KeyCode and 'KeyCode' or 'UserInputType',
								input.Name
							}
                            --
			...
			library.began[#library.began + 1] = function(Input)
				...
				if Input.UserInputType == Enum.UserInputType.MouseButton1 and window.isVisible and keybind_outline.Visible then
					if utility:MouseOverDrawing({
						section.section_frame.Position.X + (section.section_frame.Size.X - (46)),
				...
				if Input.UserInputType == Enum.UserInputType.MouseButton2 and window.isVisible and keybind_outline.Visible then
					if utility:MouseOverDrawing({
						...
						keybind.open = not keybind.open
                        --
						local modemenu = utility:Create('Frame', {
							Vector2.new(keybind_outline.Size.X + 2, 0),
							keybind_outline
						}, {
                        ...
                        --
						local modemenu_inline = utility:Create('Frame', {
							Vector2.new(1, 1),
							modemenu
						}, {
                        ...
                        --
						local modemenu_frame = utility:Create('Frame', {
							Vector2.new(1, 1),
							modemenu_inline
						}, {
						...
						for i, v in pairs({
							...
							local button_title = utility:Create('TextLabel', {
								Vector2.new(modemenu_frame.Size.X / 2, 15 * (i - 1)),
								modemenu_frame
							}, {
								Text = v,
								Size = theme.textsize,
	...
	function sections:Slider(info)
		...
		local decimals = info.increments or info.inc or info.decimals or info.Decimals or 1
		decimals = 1 / decimals
		local pointer = info.pointer or info.Pointer or info.flag or info.Flag or nil
		local callback = info.callback or info.callBack or info.Callback or info.CallBack or function()
		end
        ...
        --
		local slider_outline = utility:Create('Frame', {
			Vector2.new(4, slider.axis + 15),
			section.section_frame
		}, {
        ...
        --
		local slider_inline = utility:Create('Frame', {
			Vector2.new(1, 1),
			slider_outline
		}, {
        ...
        --
		local slider_slide = utility:Create('Frame', {
			Vector2.new(1, 1),
			slider_inline
		}, {
        ...
        --
		local slider__gradient = utility:Create('Image', {
			Vector2.new(0, 0),
			slider_frame
		}, {
		...
		local slider_value = utility:Create('TextLabel', {
			Vector2.new(slider_outline.Size.X / 2, (slider_outline.Size.Y / 2) - (textBounds.Y / 2)),
			slider_outline
		}, {
			Text = slider.current .. slider.sub,
			Size = theme.textsize,
			Font = theme.font,
			Color = theme.textcolor,
			Center = true,
		...
		function slider:set(value)
			...
			slider_value.Text = slider.current .. slider.sub
        ...
        --
		library.began[#library.began + 1] = function(Input)
			if Input.UserInputType == Enum.UserInputType.MouseButton1 and slider_outline.Visible and window.isVisible and page.open and utility:MouseOverDrawing({
				section.section_frame.Position.X,
				section.section_frame.Position.Y + slider.axis,
				section.section_frame.Position.X + section.section_frame.Size.X,
				section.section_frame.Position.Y + slider.axis + 27
			}) and not window:IsOverContent() then
				slider.holding = true
				slider:Refresh()
			end
	...
	function sections:Button(info)
        ...
        --
		local button_outline = utility:Create('Frame', {
			Vector2.new(4, button.axis),
			section.section_frame
		}, {
        ...
        --
		local button_inline = utility:Create('Frame', {
			Vector2.new(1, 1),
			button_outline
		}, {
        ...
        --
		local button_frame = utility:Create('Frame', {
			Vector2.new(1, 1),
			button_inline
		}, {
        ...
        --
		local button_title = utility:Create('TextLabel', {
			Vector2.new(button_frame.Size.X / 2, 1),
			button_frame
		}, {
			Text = name,
			Size = theme.textsize,
        ...
        --
		library.began[#library.began + 1] = function(Input)
			if Input.UserInputType == Enum.UserInputType.MouseButton1 and button_outline.Visible and window.isVisible and utility:MouseOverDrawing({
				section.section_frame.Position.X,
				section.section_frame.Position.Y + button.axis,
				section.section_frame.Position.X + section.section_frame.Size.X,
				section.section_frame.Position.Y + button.axis + 20
			}) and not window:IsOverContent() then
	...
	function sections:ButtonHolder(info)
		...
		for i = 1, 2 do
            ...
            --
			local button_outline = utility:Create('Frame', {
				Vector2.new(i == 2 and ((section.section_frame.Size.X / 2) + 2) or 4, button.axis),
				section.section_frame
			}, {
			...
			library.began[#library.began + 1] = function(Input)
				if Input.UserInputType == Enum.UserInputType.MouseButton1 and button_outline.Visible and window.isVisible and utility:MouseOverDrawing({
					section.section_frame.Position.X + (i == 2 and (section.section_frame.Size.X / 2) or 0),
					section.section_frame.Position.Y + button.axis,
					section.section_frame.Position.X + section.section_frame.Size.X - (i == 1 and (section.section_frame.Size.X / 2) or 0),
					section.section_frame.Position.Y + button.axis + 20
				}) and not window:IsOverContent() then
					buttons[i][2]()
				end
	...
	function sections:Dropdown(info)
		...
		local options = info.options or info.Options or {
			'1',
			'2',
			'3'
		}
        ...
        --
		local dropdown_outline = utility:Create('Frame', {
			Vector2.new(4, dropdown.axis + 15),
			section.section_frame
		}, {
			Size = utility:Size(1, -8, 0, 20, section.section_frame),
        ...
        --
		local dropdown_inline = utility:Create('Frame', {
			Vector2.new(1, 1),
			dropdown_outline
		}, {
        ...
        --
		local dropdown_frame = utility:Create('Frame', {
			Vector2.new(1, 1),
			dropdown_inline
		}, {
        ...
        --
		local dropdown_title = utility:Create('TextLabel', {
			Vector2.new(4, dropdown.axis),
			section.section_frame
		}, {
			Text = name,
			Size = theme.textsize,
        ...
        --
		local dropdown__gradient = utility:Create('Image', {
			Vector2.new(0, 0),
			dropdown_frame
		}, {
        ...
        --
		local dropdown_value = utility:Create('TextLabel', {
			Vector2.new(3, dropdown_frame.Size.Y / 2 - 7),
			dropdown_frame
		}, {
        ...
        --
		local dropdown_image = utility:Create('Image', {
			Vector2.new(dropdown_frame.Size.X - 15, dropdown_frame.Size.Y / 2 - 3),
			dropdown_frame
		}, {
		...
		dropdown['dropdown_image'] = dropdown_image
        --
		...
		function dropdown:Update()
			if dropdown.open and dropdown.holder.inline then
				for i, v in pairs(dropdown.holder.buttons) do
					...
					utility:UpdateOffset(v[1], {
						Vector2.new(v[1].Text == tostring(dropdown.current) and 8 or 6, 2),
						v[2]
					})
				end
			end
		...
		library.began[#library.began + 1] = function(Input)
			if Input.UserInputType == Enum.UserInputType.MouseButton1 and window.isVisible and dropdown_outline.Visible then
				if dropdown.open and dropdown.holder.inline and utility:MouseOverDrawing({
					dropdown.holder.inline.Position.X,
					dropdown.holder.inline.Position.Y,
					dropdown.holder.inline.Position.X + dropdown.holder.inline.Size.X,
					dropdown.holder.inline.Position.Y + dropdown.holder.inline.Size.Y
				}) then
					for i, v in pairs(dropdown.holder.buttons) do
						if utility:MouseOverDrawing({
							v[2].Position.X,
							v[2].Position.Y,
							v[2].Position.X + v[2].Size.X,
							v[2].Position.Y + v[2].Size.Y
						}) and v[1].Text ~= dropdown.current then
				...
				elseif utility:MouseOverDrawing({
					section.section_frame.Position.X,
					section.section_frame.Position.Y + dropdown.axis,
					section.section_frame.Position.X + section.section_frame.Size.X,
					section.section_frame.Position.Y + dropdown.axis + 15 +  20
				}) and not window:IsOverContent() then
					if not dropdown.open then
                        ...
                        --
						local dropdown_open_outline = utility:Create('Frame', {
							Vector2.new(0, 19),
							dropdown_outline
						}, {
						...
						dropdown.holder.outline = dropdown_open_outline
                        --
						library.colors[dropdown_open_outline] = {
							Color = 'outline'
						}
                        --
						local dropdown_open_inline = utility:Create('Frame', {
							Vector2.new(1, 1),
							dropdown_open_outline
						}, {
						...
						dropdown.holder.inline = dropdown_open_inline
                        --
						...
						for i, v in pairs(options) do
							local dropdown_value_frame = utility:Create('Frame', {
								Vector2.new(1, 1 + (19 * (i - 1))),
								dropdown_open_inline
							}, {
                            ...
                            --
							local dropdown_value = utility:Create('TextLabel', {
								Vector2.new(v == tostring(dropdown.current) and 8 or 6, 2),
								dropdown_value_frame
							}, {
								Text = v,
								Size = theme.textsize,
								Font = theme.font,
							...
							dropdown.holder.buttons[#dropdown.holder.buttons + 1] = {
								dropdown_value,
								dropdown_value_frame
							}
                            --
	...
	function sections:Multibox(info)
        ...
        --
		local multibox_outline = utility:Create('Frame', {
			Vector2.new(4, multibox.axis + 15),
			section.section_frame
		}, {
			Size = utility:Size(1, -8, 0, 20, section.section_frame),
        ...
        --
		local multibox_inline = utility:Create('Frame', {
			Vector2.new(1, 1),
			multibox_outline
		}, {
        ...
        --
		local multibox_frame = utility:Create('Frame', {
			Vector2.new(1, 1),
			multibox_inline
		}, {
        ...
        --
		local multibox_title = utility:Create('TextLabel', {
			Vector2.new(4, multibox.axis),
			section.section_frame
		}, {
			Text = name,
			Size = theme.textsize,
        ...
        --
		local multibox__gradient = utility:Create('Image', {
			Vector2.new(0, 0),
			multibox_frame
		}, {
        ...
        --
		local multibox_value = utility:Create('TextLabel', {
			Vector2.new(3, multibox_frame.Size.Y / 2 - 7),
			multibox_frame
		}, {
			Text = '',
			Size = theme.textsize,
        ...
        --
		local multibox_image = utility:Create('Image', {
			Vector2.new(multibox_frame.Size.X - 15, multibox_frame.Size.Y / 2 - 3),
			multibox_frame
		}, {
		...
		multibox['multibox_image'] = multibox_image
        --
		...
		function multibox:Update()
			if multibox.open and multibox.holder.inline then
				for i, v in pairs(multibox.holder.buttons) do
                    ...
                    --
					utility:UpdateOffset(v[1], {
						Vector2.new(table.find(multibox.current, v[1].Text) and 8 or 6, 2),
						v[2]
					})
				end
			end
		...
		library.began[#library.began + 1] = function(Input)
			if Input.UserInputType == Enum.UserInputType.MouseButton1 and window.isVisible and multibox_outline.Visible then
				if multibox.open and multibox.holder.inline and utility:MouseOverDrawing({
					multibox.holder.inline.Position.X,
					multibox.holder.inline.Position.Y,
					multibox.holder.inline.Position.X + multibox.holder.inline.Size.X,
					multibox.holder.inline.Position.Y + multibox.holder.inline.Size.Y
				}) then
					for i, v in pairs(multibox.holder.buttons) do
						if utility:MouseOverDrawing({
							v[2].Position.X,
							...
							v[2].Position.X + v[2].Size.X,
							v[2].Position.Y + v[2].Size.Y
						}) and v[1].Text ~= multibox.current then
				...
				elseif utility:MouseOverDrawing({
					section.section_frame.Position.X,
					section.section_frame.Position.Y + multibox.axis,
					section.section_frame.Position.X + section.section_frame.Size.X,
					section.section_frame.Position.Y + multibox.axis + 15 +  20
				}) and not window:IsOverContent() then
					if not multibox.open then
                        ...
                        --
						local multibox_open_outline = utility:Create('Frame', {
							Vector2.new(0, 19),
							multibox_outline
						}, {
						...
						multibox.holder.outline = multibox_open_outline
                        --
						library.colors[multibox_open_outline] = {
							Color = 'outline'
						}
                        --
						local multibox_open_inline = utility:Create('Frame', {
							Vector2.new(1, 1),
							multibox_open_outline
						}, {
						...
						multibox.holder.inline = multibox_open_inline
                        --
                        ...
                        --
						for i, v in pairs(options) do
							local multibox_value_frame = utility:Create('Frame', {
								Vector2.new(1, 1 + (19 * (i - 1))),
								multibox_open_inline
							}, {
                            ...
                            --
							local multibox_value = utility:Create('TextLabel', {
								Vector2.new(table.find(multibox.current, v) and 8 or 6, 2),
								multibox_value_frame
							}, {
								Text = v,
								Size = theme.textsize,
								Font = theme.font,
							...
							multibox.holder.buttons[#multibox.holder.buttons + 1] = {
								multibox_value,
								multibox_value_frame
							}
                            --
	...
	function sections:Keybind(info)
		...
		local changed = info.changed or info.Changed or function()
		end
        --
		local window = self.window
		local page = self.page
        ...
        --
		local keybind_title = utility:Create('TextLabel', {
			Vector2.new(4, keybind.axis + (8.5) - (utility:GetTextBounds(name, theme.textsize, theme.font).Y / 2)),
			section.section_frame
		}, {
			Text = name,
			Size = theme.textsize,
			...
			Color = theme.textcolor,
			OutlineColor = theme.textborder,
			Position = utility:Position(0, 4, 0, keybind.axis + (8.5) - (utility:GetTextBounds(name, theme.textsize, theme.font).Y / 2), section.section_frame),
			Visible = page.open
		}, section.visibleContent)
        --
		library.colors[keybind_title] = {
	...
	function sections:Colorpicker(info)
        ...
        --
		local colorpicker_outline = utility:Create('Frame', {
			Vector2.new(section.section_frame.Size.X - (34), colorpicker.axis),
			section.section_frame
		}, {
			Size = utility:Size(0, 30, 0, 15),
			Position = utility:Position(1, -34, 0, colorpicker.axis, section.section_frame),
			Color = theme.outline,
			Visible = page.open
		}, section.visibleContent)
		library.colors[colorpicker_outline] = {
        ...
        --
		local colorpicker_title = utility:Create('TextLabel', {
			Vector2.new(4, colorpicker.axis + (7.5) - (utility:GetTextBounds(name, theme.textsize, theme.font).Y / 2)),
			section.section_frame
		}, {
			Text = name,
			Size = theme.textsize,
			...
			Color = theme.textcolor,
			OutlineColor = theme.textborder,
			Position = utility:Position(0, 4, 0, colorpicker.axis + (7.5) - (utility:GetTextBounds(name, theme.textsize, theme.font).Y / 2), section.section_frame),
			Visible = page.open
		}, section.visibleContent)
        --
		library.colors[colorpicker_title] = {
		...
		library.began[#library.began + 1] = function(Input)
			if Input.UserInputType == Enum.UserInputType.MouseButton1 and window.isVisible and colorpicker_outline.Visible then
				...
				elseif utility:MouseOverDrawing({
					section.section_frame.Position.X,
					section.section_frame.Position.Y + colorpicker.axis,
					section.section_frame.Position.X + section.section_frame.Size.X - (colorpicker.secondColorpicker and (34) or 0),
					section.section_frame.Position.Y + colorpicker.axis + 15
				}) and not window:IsOverContent() then
					if not colorpicker.open then
						window:CloseContent()
		...
		function colorpicker:Colorpicker(info)
            ...
            --
			colorpicker_outline.Position = utility:Position(1, -68, 0, colorpicker.axis, section.section_frame)
			utility:UpdateOffset(colorpicker_outline, {
				Vector2.new(section.section_frame.Size.X - (68), colorpicker.axis),
				section.section_frame
			})
            --
			local colorpicker_outline = utility:Create('Frame', {
			...
			library.began[#library.began + 1] = function(Input)
				if Input.UserInputType == Enum.UserInputType.MouseButton1 and window.isVisible and colorpicker_outline.Visible then
					...
					elseif utility:MouseOverDrawing({
						section.section_frame.Position.X + (section.section_frame.Size.X - (36)),
						section.section_frame.Position.Y + colorpicker.axis,
						section.section_frame.Position.X + section.section_frame.Size.X,
						section.section_frame.Position.Y + colorpicker.axis + 15
					}) and not window:IsOverContent() then
	...
	function sections:Listbox(info)
        ...
        --
		local listbox_outline = utility:Create('Frame', {
			Vector2.new(4, listbox.axis),
			section.section_frame
		}, {
        ...
        --
		local listbox_inline = utility:Create('Frame', {
			Vector2.new(1, 1),
			listbox_outline
		}, {
        ...
        --
		local listbox_frame = utility:Create('Frame', {
			Vector2.new(1, 1),
			listbox_inline
		}, {
        ...
        --
		local listbox_gradient = utility:Create('Image', {
			Vector2.new(0, 0),
			listbox_frame
		}, {
        ...
        --
		local listbox_scroll_outline = utility:Create('Frame', {
			Vector2.new(listbox_gradient.Size.X - 4, 0),
			listbox_gradient
		}, {
        ...
        --
		local listbox_scroll_frame = utility:Create('Frame', {
			Vector2.new(1, 1),
			listbox_scroll_outline
		}, {
		...
		function listbox:RefreshScroll(down)
			for i = self.startindex, 1, -1 do
				...
				utility:UpdateOffset(button, {
					Vector2.new(listbox_frame.Size.X / 2, 2 + 18 * (#self.buttons - i)),
					listbox_frame
				})
                --
			...
			for i = self.startindex, 7 + self.startindex do
				...
				utility:UpdateOffset(button, {
					Vector2.new(listbox_frame.Size.X / 2, 2 + 18 * (i - self.startindex)),
					listbox_frame
				})
                --
		...
		function listbox:RefreshScrollBar()
			...
			utility:UpdateOffset(listbox_scroll_frame, {
				Vector2.new(1, 1 + scale * listbox_scroll_outline.Size.Y),
				listbox_scroll_outline
			})
		...
		function listbox:UpdateList(list, dontset, keepscroll)
			...
			for i, name in ipairs(list) do
				local button_title = utility:Create('TextLabel', {
					Vector2.new(listbox_frame.Size.X / 2, 2 + (18 * #listbox.buttons)),
					listbox_frame
				}, {
					Text = name,
					Size = theme.textsize,
	...
	function sections:ConfigBox(info)
        ...
        --
		local configLoader_outline = utility:Create('Frame', {
			Vector2.new(4, configLoader.axis),
			section.section_frame
		}, {
			Size = utility:Size(1, -8, 0, 148, section.section_frame),
        ...
        --
		local configLoader_inline = utility:Create('Frame', {
			Vector2.new(1, 1),
			configLoader_outline
		}, {
        ...
        --
		local configLoader_frame = utility:Create('Frame', {
			Vector2.new(1, 1),
			configLoader_inline
		}, {
        ...
        --
		local configLoader_gradient = utility:Create('Image', {
			Vector2.new(0, 0),
			configLoader_frame
		}, {
        ...
        --
		for i = 1, 8 do
			local config_title = utility:Create('TextLabel', {
				Vector2.new(configLoader_frame.Size.X / 2, 2 + (18 * (i - 1))),
				configLoader_frame
			}, {
        ...
        --
		library.began[#library.began + 1] = function(Input)
			if Input.UserInputType == Enum.UserInputType.MouseButton1 and configLoader_outline.Visible and window.isVisible and utility:MouseOverDrawing({
				section.section_frame.Position.X,
				section.section_frame.Position.Y + configLoader.axis,
				section.section_frame.Position.X + section.section_frame.Size.X,
				section.section_frame.Position.Y + configLoader.axis + 148
			}) and not window:IsOverContent() then
				for i = 1, 8 do
					if utility:MouseOverDrawing({
						section.section_frame.Position.X,
						section.section_frame.Position.Y + configLoader.axis + 2 + (18 * (i - 1)),
						section.section_frame.Position.X + section.section_frame.Size.X,
						section.section_frame.Position.Y + configLoader.axis + 2 + (18 * (i - 1)) + 18
					}) then
